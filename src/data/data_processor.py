...
def _build_messages(item: Dict[str, Any], base_path: Path, mode: str, sys_enable: bool) -> List[Dict[str, Any]]:
...
def preprocess_qwen_visual(
    ...
    assert mode in ["supporting_conversations",
                    "contradictory_conversations"], f"mode {mode} not supported, must in ['supporting_conversations', 'contradictory_conversations']."
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, processor, data_args):  # input: image processor and data args
        super(LazySupervisedDataset, self).__init__()

        dataset = data_args.dataset_use.split(",")  # split multiple datasets
        # get dataset configs (annotation_path, data_path(image_path), sampling_rate)
        ...
        for data in dataset_list:
            ...
            # add data path to each annotation
        ...
        # Randomly shuffle the data for training
    ...
    def _get_item(self, sources) -> Dict[str, torch.Tensor]:
        ...
        label = self.processor.tokenizer.decode(
            labels, skip_special_tokens=False)

        # for contradictory conversations
        seq_len = data_dict_cont["input_ids"][0].size(0)

        position_ids, _ = self.get_rope_index(
            self.merge_size,
            data_dict_cont["input_ids"],
            image_grid_thw=torch.cat(grid_thw, dim=0) if grid_thw else None,
            video_grid_thw=(
                torch.cat(video_grid_thw, dim=0) if video_grid_thw else None
            ),
        ...
        data_dict_cont["position_ids"] = position_ids
        data_dict_cont["attention_mask"] = [seq_len]

        # save contradictory conversations data, image related data are shared
        data_dict["input_ids_cont"] = data_dict_cont["input_ids"]
        data_dict["labels_cont"] = data_dict_cont["labels"]

        left_id = 508  # " ["
        right_id = 1125  # "],"

        cont_labels = data_dict_cont["labels"][0].clone()
        left_pos = (cont_labels == left_id).nonzero(as_tuple=True)[0]
        right_pos = (cont_labels == right_id).nonzero(as_tuple=True)[0]

        if len(left_pos) == 0 or len(right_pos) == 0:
            raise ValueError("No bounding box tokens found in the labels.")
        else:
            l = left_pos[0].item()
            r = right_pos[0].item()
            ...
            if l >= r:
                raise ValueError(
                    "Left bracket position is after right bracket position.")

            bbox_only_labels = torch.full_like(cont_labels, IGNORE_INDEX)
            no_bbox_labels = torch.full_like(cont_labels, IGNORE_INDEX)

            # only keep bbox parts
            bbox_only_labels[l+1:r] = cont_labels[l+1:r]
            no_bbox_labels[0:l+1] = cont_labels[0:l+1]
            no_bbox_labels[r:] = cont_labels[r:]  # only keep non-bbox parts

        data_dict["labels_cont_bbox_only"] = bbox_only_labels.squeeze(0)
        data_dict["labels_cont_no_bbox"] = no_bbox_labels.squeeze(0)

        data_dict["position_ids_cont"] = data_dict_cont["position_ids"]
        data_dict["attention_mask_cont"] = data_dict_cont["attention_mask"]
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        # for contradictory conversations
        input_ids_cont, labels_cont, position_ids_cont = tuple(
            [instance[key] for instance in instances]
            for key in ("input_ids_cont", "labels_cont", "position_ids_cont")
        )
        if "labels_cont_bbox_only" in instances[0]:
            labels_cont_bbox_only, labels_cont_no_bbox = tuple(
                [instance[key] for instance in instances]
                for key in ("labels_cont_bbox_only", "labels_cont_no_bbox")
            )
            labels_cont_bbox_only = [ids.squeeze(0)
                                     for ids in labels_cont_bbox_only]
            labels_cont_no_bbox = [ids.squeeze(0)
                                   for ids in labels_cont_no_bbox]
            labels_cont_bbox_only = torch.nn.utils.rnn.pad_sequence(
                labels_cont_bbox_only, batch_first=True, padding_value=IGNORE_INDEX
            )
            labels_cont_no_bbox = torch.nn.utils.rnn.pad_sequence(
                labels_cont_no_bbox, batch_first=True, padding_value=IGNORE_INDEX
            )
            labels_cont_bbox_only = labels_cont_bbox_only[:,
                                                          : self.tokenizer.model_max_length]
            labels_cont_no_bbox = labels_cont_no_bbox[:,
                                                      : self.tokenizer.model_max_length]
            batch["labels_cont_bbox_only"] = labels_cont_bbox_only
            batch["labels_cont_no_bbox"] = labels_cont_no_bbox
        input_ids_cont = [ids.squeeze(0) for ids in input_ids_cont]
        labels_cont = [ids.squeeze(0) for ids in labels_cont]
        input_ids_cont = torch.nn.utils.rnn.pad_sequence(
            input_ids_cont, batch_first=True, padding_value=self.tokenizer.pad_token_id
        )
        labels_cont = torch.nn.utils.rnn.pad_sequence(
            labels_cont, batch_first=True, padding_value=IGNORE_INDEX
        )
        position_ids_cont = pad_and_cat(position_ids_cont)
        input_ids_cont = input_ids_cont[:, : self.tokenizer.model_max_length]
        labels_cont = labels_cont[:, : self.tokenizer.model_max_length]
        position_ids_cont = position_ids_cont[:,
                                              :, : self.tokenizer.model_max_length]
        batch["input_ids_cont"] = input_ids_cont
        batch["labels_cont"] = labels_cont
        batch["position_ids_cont"] = position_ids_cont
        batch["attention_mask_cont"] = input_ids_cont.ne(
